...
The backend connects to a **local** Postgres via `DATABASE_URL`
(`postgresql://postgres:password@...`), started by `start.sh`. This is NOT the
same database that the `executeSql` code-execution callback or the `database`
skill query — those hit the Replit-managed Postgres.
