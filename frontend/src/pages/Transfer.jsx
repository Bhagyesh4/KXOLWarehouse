...
function StatusBadge({ status }) {
    if (status === "confirmed")
        return (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">
                <CheckCircle2 size={9} /> Confirmed
            </span>
        );
    return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider bg-amber-500/15 text-amber-400 border border-amber-500/30">
            <ClipboardCheck size={9} /> Pending
        </span>
    );
}

function TransferLabelModal({ transfer, onClose }) {
    const handlePrint = () => window.print();
    const palletVal = transfer.pallet_code || transfer.sku_code;
    const locVal = transfer.to_code;
        ...
        <>
            <style>{`
                @media print {
                    ...
                    #transfer-label-print,
                    #transfer-label-print * { visibility: visible !important; }
                    #transfer-label-print {
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-[#181a20] border border-white/15 w-full max-w-md flex flex-col">
                    <div className="flex items-center justify-between px-5 py-3 border-b border-white/10">
                        <div>
                            <div className="font-mono text-[10px] uppercase tracking-widest text-amber-400">Transfer Label</div>
                            <div className="text-sm font-bold mt-0.5">Scan to Confirm Putaway</div>
                        </div>
                    ...
                    <div id="transfer-label-print" className="bg-white text-black p-6 mx-5 my-5">
                        <div className="text-center mb-4">
                            <div className="text-[10px] font-mono uppercase tracking-[0.25em] text-gray-500 mb-0.5">
                                FrostCore WMS · Transfer Label
                            </div>
                        ...
                        <div className="border-b border-dashed border-gray-300 mb-4 pb-4">
                            <div className="text-[9px] font-mono uppercase tracking-widest text-gray-500 text-center mb-1">
                                Pallet Code
                            </div>
                            <div className="flex justify-center">
                                {palletVal ? (
                                    ...
                                    <div className="text-gray-400 text-xs py-4">No pallet code</div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="border-t border-dashed border-gray-300 pt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                            <div>
                                <span className="text-[9px] font-mono uppercase text-gray-400">SKU</span>
                                <div className="font-mono font-bold">{transfer.sku_code}</div>
                                <div className="text-gray-500 text-[10px]">{transfer.sku_name}</div>
                            </div>
                            <div>
                                <span className="text-[9px] font-mono uppercase text-gray-400">Qty</span>
                                <div className="font-mono font-bold text-lg">{transfer.qty}</div>
                            </div>
                            <div>
                                <span className="text-[9px] font-mono uppercase text-gray-400">From</span>
                                <div className="font-mono">{transfer.from_code}</div>
                            </div>
                            <div>
                                <span className="text-[9px] font-mono uppercase text-gray-400">To</span>
                                <div className="font-mono font-bold text-green-700">{transfer.to_code}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2 px-5 pb-5">
                        <button
                            onClick={handlePrint}
                            className="flex-1 flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 text-black px-4 py-2.5 text-sm font-bold uppercase tracking-wider"
                        >
                            <Printer size={14} />
                            Print Label
                        </button>
                        <button
                            onClick={onClose}
                            className="px-4 py-2.5 text-sm border border-white/10 text-gray-400 hover:text-white hover:border-white/30"
                        >
                            Close
                        </button>
                    </div>
...
export default function Transfer() {
    ...
    const [labelTransfer, setLabelTransfer] = useState(null);
    ...
    const load = async () => {
        const [palletsRes, locsRes, histRes] = await Promise.all([
            ...
            api.get("/storage/transfers?limit=100"),
        ]);
    ...
    const pending = history.filter((h) => h.status === "initiated");
    ...
    const doTransfer = async () => {
        ...
        setBusy(true);
        setMsg(null);
        const palletSnap = { ...selectedPallet };
        const destSnap = { ...selectedDest };
        try {
            const res = await api.post("/storage/transfer", {
                stock_id: palletSnap.id,
                to_location_id: destSnap.id,
            });
    ...
    const confirmBarcode = async (val) => {
        ...
        try {
            ...
            setScanMsg({ type: "error", text: formatErr(e.response?.data?.detail) || "Scan failed" });
        } finally {
            setScanBusy(false);
    ...
    return (
        <div className="space-y-5">
            {labelTransfer && (
                <TransferLabelModal transfer={labelTransfer} onClose={() => setLabelTransfer(null)} />
            )}
                ...
                <div className="flex gap-6">
                    {pending.length > 0 && (
                        <div className="text-right">
                            <div className="font-mono text-[10px] uppercase tracking-widest text-gray-500">Pending Putaway</div>
                            <div className="text-2xl font-bold text-amber-400">{pending.length}</div>
                        </div>
                    )}
                    <div className="text-right">
                        <div className="font-mono text-[10px] uppercase tracking-widest text-gray-500">Today</div>
                        <div className="text-2xl font-bold text-white">{transfersToday}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-[#181a20] border border-white/10 flex flex-col">
                    <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
                        ...
                        {selectedPallet && (
                            <button
                                onClick={() => { setSelectedPallet(null); setSelectedDest(null); }}
                    ...
                    <div className="overflow-y-auto" style={{ maxHeight: 320 }}>
                        {filteredPallets.length === 0 ? (
                            ...
                            return (
                                <button
                                    key={p.id}
                                    onClick={() => { setSelectedPallet(sel ? null : p); setSelectedDest(null); }}
                                    className={`w-full text-left px-4 py-3 border-b border-white/5 transition-colors ${
                                        sel ? "bg-amber-500/10 border-l-2 border-l-amber-500" : "hover:bg-white/5"
                                    }`}
                                >
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="font-mono text-xs text-amber-400">{p.pallet_code || "—"}</span>
                                                {p.bag_color && (
                                                    <span
                                            ...
                                            <div className="text-xs text-gray-300 truncate mt-0.5">{p.sku_code} · {p.sku_name}</div>
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex gap-2 mt-0.5">
                                            ...
                                            <span className="text-[10px] text-gray-600">·</span>
                                            <span className="text-[10px] text-emerald-400">{free} slot{free !== 1 ? "s" : ""} free</span>
                                        </div>
                                    </div>
                ...
                <div className="bg-[#111317] border border-amber-500/30 p-4 flex flex-wrap items-center justify-between gap-4">
                    <div className="flex flex-wrap items-center gap-6">
                        <div className="text-center">
                            ...
                            <div className="font-mono text-[10px] text-amber-400 mt-0.5">{selectedPallet.pallet_code || "—"}</div>
                        </div>
                        ...
                        <div className="text-center">
                            ...
                            <div className="font-mono text-[10px] text-gray-400 mt-0.5">{selectedDest.zone_name || selectedDest.zone}</div>
                        </div>
                        <div className="text-center">
            ...
            {!canTransfer && (
                <div className="border border-white/10 bg-white/5 px-4 py-2 text-xs text-gray-500 font-mono">
                    Operators can view and confirm putaway but cannot initiate transfers.
                </div>
            )}

            {/* Confirm Putaway Scanner */}
            <div className="bg-[#181a20] border border-white/10">
                <div className="px-5 py-3 border-b border-white/10 flex items-center justify-between">
                    <div>
                        ...
                        <div className="text-sm font-bold mt-0.5 flex items-center gap-2">
                            ...
                            {pending.length > 0 && (
                                <span className="font-mono text-xs bg-amber-500/20 text-amber-400 border border-amber-500/30 px-2 py-0.5">
                                    {pending.length} pending
                                </span>
                            )}
                        </div>
                    </div>
                    <ScanLine size={18} className="text-gray-500" />
                </div>
                ...
                <div className="p-4 space-y-3">
                    {pending.length > 0 && (
                        <div className="space-y-1 mb-3">
                            {pending.map((t) => (
                                <div
                                    key={t.id}
                                    className="flex items-center gap-3 bg-amber-500/5 border border-amber-500/20 px-3 py-2"
                                >
                                    <ClipboardCheck size={13} className="text-amber-400 shrink-0" />
                                    <div className="flex-1 min-w-0 grid grid-cols-3 gap-2 text-[11px]">
                                        <span className="font-mono text-amber-400 truncate">{t.pallet_code || "—"}</span>
                                        <span className="font-mono text-gray-500 truncate">{t.from_code}</span>
                                        <span className="font-mono text-emerald-400 truncate">→ {t.to_code}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {scanMsg && (
                        <div
                            className={`border px-3 py-2 text-xs flex items-center justify-between ${
                                scanMsg.type === "ok"
                                    ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300"
                                    : "border-red-500/40 bg-red-500/10 text-red-300"
                            }`}
                        >
                            ...
                            <button onClick={() => setScanMsg(null)} className="ml-3 text-gray-400 hover:text-white">
                                <X size={12} />
                            </button>
                        </div>
                    )}
                    ...
                    <form onSubmit={doScan} className="flex gap-2">
                        ...
                        <button
                            type="submit"
                            disabled={scanBusy || !scanBarcode.trim()}
                            className="px-5 py-2 bg-white/10 hover:bg-white/15 disabled:opacity-40 text-sm font-mono border border-white/10 text-gray-300"
                        >
                            {scanBusy ? "…" : "Confirm"}
                        </button>
                    </form>
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-[#111317] text-[10px] uppercase tracking-[0.15em] text-gray-500">
                            <tr>
                                ...
                                <th className="text-left py-2.5 px-4">Initiated By</th>
                        ...
                        <tbody>
                            {history.length === 0 ? (
                                ...
                                <tr key={h.id} className="border-b border-white/5 hover:bg-white/5">
                                    ...
                                    <td className="py-2.5 px-4 font-mono text-xs text-amber-400">{h.pallet_code || "—"}</td>
                                    ...
                                    <td className="py-2.5 px-4 text-xs text-gray-400">{h.transferred_by_name || "—"}</td>
                                    <td className="py-2.5 px-4 text-right font-mono text-[10px] text-gray-500">
                                        {h.transferred_at ? new Date(h.transferred_at).toLocaleString() : "—"}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
